...
- **Important:** Make sure to export your emitter (e.g. `export default emitter`) so it can be accessed by tests and other files. The project uses ESM; tests are run with Vitest.
...
The `export { app, server }` statement exports these values so they can be used by your TDD tests. The tests are run with Vitest and use Supertest, which needs the Express `app` (and optionally the `server` for cleanup). Also, `app.listen()` is asynchronous, with a callback, but it does return the value of `server` synchronously, before the listen operation has completed. That suffices for Supertest.
...
Export your function (e.g. `export default notFoundMiddleware`) and add the needed `import` and `app.use()` statements in `app.js`.  
