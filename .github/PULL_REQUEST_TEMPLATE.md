---
name: Pull Request
about: "Use this template for any type of change: bug fix, feature, documentation, task, support, or security."
---

# Description

Provide a clear and concise description of what this PR does. Include motivation, context, and any relevant background.

---

# Related Issue(s)

If this PR fixes an issue, link it here:

- Linked issue (if applicable): Closes #123 (replace with the actual issue number, or remove if not applicable)

If no issue exists, briefly describe the context or motivation for this PR below:

<!-- Explain context here -->

---

# Type of Change

Please check the primary type of change this PR represents:

- [ ] Bug fix (`bug_report.md`)
- [ ] New feature (`feature_request.md`)
- [ ] Documentation update (`documentation_issue.md`)
- [ ] Task / Chore (`task_chore.md`)
- [ ] Refactor / cleanup
- [ ] Test update
- [ ] Security fix (`security_issue.md`)

---

# Breaking Changes

- [ ] This PR introduces breaking changes
- [ ] No breaking changes

---

# Checklist

- [ ] I have read the [CONTRIBUTING.md](CONTRIBUTING.md) file
- [ ] I have tested my changes locally (if applicable)
- [ ] I have added or updated tests where needed (if applicable)
- [ ] I have updated documentation where required
- [ ] Continuous Integration (CI) checks pass
- [ ] All tests pass

---

# Additional Notes

Include any additional context, screenshots, or special instructions for reviewers.
